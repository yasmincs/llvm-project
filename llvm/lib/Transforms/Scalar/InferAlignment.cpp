...
/// Return true if \p Ptr is (or strips to) a single-index GEP, which is the
/// only case where variable-offset alignment analysis can help.
static bool hasSingleIndexGEP(Value *Ptr) {
  return isa<GEPOperator>(Ptr) &&
         cast<GEPOperator>(Ptr)->getNumIndices() == 1;
}
...
static std::optional<Align> getIndexAlignmentFromPattern(Value *Idx) {
  ...
  Value *CastSrc;
  if (match(Idx, m_SExt(m_Value(CastSrc))) || match(Idx, m_ZExt(m_Value(CastSrc))))
...
bool inferAlignment(Function &F, AssumptionCache &AC, DominatorTree &DT) {
  ...
  auto updateBestBaseAlign = [&](Value *BasePtr, Align NewAlign) {
    auto [It, Inserted] = BestBasePointerAligns.try_emplace(BasePtr, NewAlign);
    if (!Inserted && NewAlign > It->second)
      It->second = NewAlign;
  };
    ...
    if (!Inserted) {
      if (It->second > BasePointerAlign) {
        Align BetterLoadStoreAlign =
            commonAlignment(It->second, ConstOffsetVal);
        if (!hasSingleIndexGEP(ConstBasePtr))
          return BetterLoadStoreAlign;
      } else if (BasePointerAlign > It->second) {
        It->second = BasePointerAlign;
      }
    }

    Align ConstFinalAlign = computeFinalAlign(ConstBasePtr, LoadStoreAlign, true,
                                              ConstOffsetVal, Align(1));
